...
function QueryTuan(activeId, tuanId) {
  return new Promise((resolve) => {
    let url = `https://m.jingxi.com/dreamfactory/tuan/QueryTuan?activeId=${escape(activeId)}&tuanId=${escape(tuanId)}&_time=${Date.now()}&_=${Date.now()}&sceneval=2&g_login_type=1&_ste=1&_stk=_time,activeId,tuanId`
    url += `&h5st=${decrypt(Date.now(), '_time,activeId,tuanId', '', url)}`
    const options = {
      'url': url,
      "headers": {
        "Accept": "*/*",
...
function CreateTuan() {
  return new Promise((resolve) => {
    let url = `https://m.jingxi.com/dreamfactory/tuan/CreateTuan?activeId=${escape(tuanActiveId)}&isOpenApp=1&_time=${Date.now()}&_=${Date.now()}&sceneval=2&g_login_type=1&_stk=_time,activeId,isOpenApp&_ste=1`
    url += `&h5st=${decrypt(Date.now(), '_time,activeId,isOpenApp', '', url)}`
    const options = {
      'url': url,
      "headers": {
        "Accept": "*/*",
...
function QueryAllTuan() {
  return new Promise((resolve) => {
    let url =`https://m.jingxi.com/dreamfactory/tuan/QueryAllTuan?activeId=${escape(tuanActiveId)}&pageNo=1&pageSize=10&_time=${Date.now()}&_=${Date.now()}&sceneval=2&g_login_type=1&_ste=1`
    url += `&h5st=${decrypt(Date.now(), '', '', url)}`
    const options = {
      'url': url,
      "headers": {
...
function tuanAward(activeId, tuanId, isTuanLeader = true) {
  return new Promise((resolve) => {
    let url = `https://m.jingxi.com/dreamfactory/tuan/Award?activeId=${escape(activeId)}&tuanId=${escape(tuanId)}&_time=${Date.now()}&_=${Date.now()}&sceneval=2&g_login_type=1&_ste=1`
    url += `&h5st=${decrypt(Date.now(), '', '', url)}`
    const options = {
      'url': url,
      "headers": {
  ...
  if (typeof URL != "undefined") {
    ...
    if (url_string.indexOf("?") != -1) {    //判断是否有参数
      ...
      for (var i = 0; i < strs.length; i++) {
        strSub = strs[i].split("=");
        if (strSub[0] == param) {
          return strs[i].substr(strs[i].indexOf('=')+1);
        }
      }
    }
    return "";
